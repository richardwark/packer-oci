...
**Note:** This workshop runs from a cloud instance to minimize changes/additions to the students laptop.  You install packer on a Compute instance in the cloud.  If you would like to install (packer & git) locally, the same instructions will work with little/no modification, on both Mac and Windows machines.
...
- If you haven't already, please request your Oracle Cloud Trial Account.
...
Using git is very common for cloud development, it provides source control and management of software, configuration and documenation files.  For the purposes of this workshop, you make use of GitHub for its ability to store, view and edit important files used in image creation - details allowing packer to build in your OCI environment
...
- call it pb_<yourTenant>.json (since it will be a docker developer's desktop image) by *typing* that `pb_<yourTentant>.json` in the text box
...
This will be referred to as the "_variables.json_" file in the future and you can use it as a template for other projects in the same tenancy.  If you wanted to build in another compartment, you just change the compartment OCID and save the file with a new name.

As you navigate around Oracle Cloud Infrastructure, you will want to keep your _variables.json_ file open to collect the required identifiers.  As you create compartments and networks, you be asked to paste identifiers into this file.  While is seems daunting, it only takes a couple of minutes.
...
- **Copy** This value and **Paste** into your _variables.json_ file's, compartment:

  ![](images/Lab100/3.11.PNG)

### **STEP 6**: Create a Virtual Cloud Network
...
### **STEP 7**: Collect Packer-build subnet OCID

- Now you can **copy** one of the subnet's OCID to the _variables.json_ file from step 3 by using the *copy* link.  
  ...
  - Back in your browser window, update your _variables.json_ file. Highlight `pasteYourSubnetsOCID`, **pasting** the OCID value after:

  ![](images/Lab100/5.5.PNG)    

Now you have your compartment and subnet defined in your variables file.

### **STEP 8**: Add a Security List entry
...
### **STEP 9 Mac/Linux**: Create SSH Key Pair (Linux or Mac client)
...
**NOTE:** `This step focuses on key pair generation for Linux or Mac based terminal sessions. If your going to run your terminal sessions from a Windows client then skip to STEP 10`
...
### **STEP 10 Windows**: Create SSH Key Pair (Windows client)

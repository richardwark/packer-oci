# Packer Workshop - Exploring OCI, Launch Packer Instance
...
***Note:  The images build date you see, might be more recent than the capture below.***

- Scroll down furthur on the page and select your PUBLIC SSH Key

**NOTE:** A public key (at least one) is deployed to each OCI cloud instance.  This allows you to connect to the running instance, as the user the public key was deployed to, with the private key held secure on your laptop.  It can be pasted or cho paste the public key created in Step 7 of lab 100 into the SSH key field by selecting the "Paste SSH keys" radio button. `The public key should be pasted as a single line`
...
The last set up piece will be to SSH into the Compute image and install packer and git.  We will use the private ssh key to connect with the newly started image.  Any SSH client can be used, but the examples here are the most common, Putty (for Windows) and built-in ssh for Linux/Mac Laptops.
...
On the Compute instance, where packer is installed, we will need to create trust with our cloud.  We will create a second keypair, RSA in .pem format used to authenticate our use of APIs.  We will upload the public piece of this key to our user and place the resulting fingerprint in our _variables_ file in the GitHub editor.            

- Do this by running  by running the **openssl genrsa** command, specifying ~/.oci (a directory under the opc users's home directory), in the command: 
...
- Now, **Copy** the string after "Fingerprint:" into the _variables_ file in your GitHub editor
...
- After **pasting** the fingerprint to your _variables_ file in your GitHub editory, , It should now look similar to: 
...
- **Copy** the OCID and paste into _variables_ in GitHub
...
- You should now have a completed _variables_ file:

  ![](images/Lab200/5.1.4.png)

- **Commit** your work on that file to your repository.  You be using in all the remaining labs.
...
- Edit SharedOS/changepasswordVNC.sh to change the password from "Oracle123".  Please choose something that you will remember.
